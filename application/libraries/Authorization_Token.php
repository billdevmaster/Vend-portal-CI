...
class Authorization_Token
{
     ...
     * @return : user informations
     */
    public function validateTokenFromSession($token_data,$clientId)
    {
        try {
            /**
             * Token Decode
            ...
            if (!empty($token_decode) and is_object($token_decode)) {
                // Check User ID (exists and numeric)
                //log_message('debug','Client ID : '.$token_decode->client_id);
                if(empty($token_decode->client_id) or $token_decode->client_id != $clientId){
                log_message('debug','Client ID mismatch Token Client ID : '.$token_decode->client_id);

                    return False;
                }
                else if (empty($token_decode->username) or !is_numeric($token_decode->username)) {
                    return False;
                    // Check Token Time
                } else if (empty($token_decode->time or !is_numeric($token_decode->time))) {
     ...
     */
    public function validateAdminTokenFromSession($token_data,$clientId)
    {
        try {
            /**
             * Token Decode
            ...
            if (!empty($token_decode) and is_object($token_decode)) {
                // Check User ID (exists and numeric)
                //log_message('debug','Client ID : '.$token_decode->client_id);
                if(empty($token_decode->client_id) or $token_decode->client_id != $clientId or intval($token_decode->client_id) != -1){
                log_message('debug','Client ID mismatch Token Client ID : '.$token_decode->client_id);
