...
class Feedback_controller extends CI_Controller
{
    ...
    public function getFeedbackData()
    {
        if ($this->checkAdminToken($this->input->request_headers())) {
            log_message('debug', 'getFeedbackData');
            $this->db->order_by('timestamp', 'DESC');
            $result = $this->db->get('feedback')->result();
            $arr_data = array();
            $i = 0;
            foreach ($result as $row) {
                $arr_data[$i]['feedback_id'] = $row->feedback_id;
                ...
                $arr_data[$i]['customer_name'] = $row->customer_name;
                $arr_data[$i]['customer_phone'] = $row->customer_phone;
                $arr_data[$i]['customer_email'] = $row->customer_email;
                $arr_data[$i]['complaint'] = $row->complaint;
                $arr_data[$i]['feedback'] = $row->feedback;
    ...
    public function getFeedbackDataByClientId()
    {
    ...
    public function getFeedbackDataMachineWise()
    {
        if ($this->checkToken($this->input->request_headers())) {
            log_message('debug', 'getFeedbackDataMachineWise');
            $this->db->order_by('timestamp', 'DESC');
