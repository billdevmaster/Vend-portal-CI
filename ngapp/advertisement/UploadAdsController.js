...
app.controller('UploadAdsController',
    ['$rootScope', '$scope', 'Upload', '$timeout', '$location', '$window', '$http', 'Base64',
        function ($rootScope, $scope, Upload, $timeout, $location, $window, $http, Base64) {
            ...
            $scope.uploadFiles = function (file, errFiles) {
                ...
                if (file) {
                    ...
                    file.upload = Upload.upload({
                        url: '/index.php/Advertisement_controller/uploadFile',
                        method: 'POST',
                        file: file,
            ...
            $scope.uploadAd = function () {
                console.log('uploadAd Called');
                var fd = new FormData();
                fd.append('ads_path', $scope.filePath);
                fd.append('ads_name', $scope.ads_name);
                if ($rootScope.isClient == true || $rootScope.isClient == 'true') {
                ...
                fd.append('ads_resolution', '.mp4 file, Max File size 20 MB, landscape, Width * Height (1280*720)');

                $http.post('/index.php/Advertisement_controller/uploadAdvertisement', fd, {
                    transformRequest: angular.identity,
                    headers: { 'Content-Type': undefined , 'Authorization': $rootScope.token, 'ClientId' : $rootScope.clientId  }
                })
                    .success(function (result) {
